    ...
    if (isStaticCF || isStaticMeta) {
        ...
        if (type === 'HTTPS') {
            ...
            if (isStaticCF) {
                ...
                ipv4Hints = [...new Set(ipv4Hints)].slice(0, 8);
                ipv6Hints = [...new Set(ipv6Hints)].slice(0, 8);
    ...
    if (data.Answer) {
        if (type === 'HTTPS') {
            ...
            if (aData && aData.Answer) {
                ipv4Hints = aData.Answer.filter(r => r.type === 1).map(r => r.data).slice(0,8);
            }
            if (aaaaData && aaaaData.Answer) {
                ipv6Hints = aaaaData.Answer.filter(r => r.type === 28).map(r => r.data).slice(0,8);
            }
        } else {
            answers = data.Answer.filter(r => r.type === dnsType).map(r => r.data).slice(0,8);
        }
    }
...
async function resolveMultiDomainToIps(domainsStr, type) {
    ...
    const promises = domains.map(async (domain) => {
        return resolveDomainToIp(domain, type);
    });
...
async function queryUpstreamDNS(name, type) {
    ...
    if (result && typeof caches !== 'undefined' && caches.default) {
        try {
            ...
            caches.default.put(cacheKey, responseToCache).catch(() => {});
        } catch (e) {}
    }
...
function parseHttpsRecord(dataStr) {
    ...
    for (let i = 2; i < parts.length; i++) {
        ...
        else if (k === 'ipv4hint') result.ipv4hint = v;
        else if (k === 'ipv6hint') result.ipv6hint = v;
    }
    return result;
}
...
function packHttpsParamsWithHints(priority, target, params, ipv4Hints, ipv6Hints) {
    if (ipv4Hints && ipv4Hints.length > 0) {
        const unique = [...new Set(ipv4Hints)].slice(0,8);
        params.push({ key: 'ipv4hint', val: unique.join(',') });
    }
    if (ipv6Hints && ipv6Hints.length > 0) {
        const unique = [...new Set(ipv6Hints)].slice(0,8);
...
async function fetchCleanEchRdataWithHints(config, domain, ctx) {
    ...
    if (config.ip4) {
        ipv4Hints = parseIpList(config.ip4);
    } else if (config.cfDomain) {
        const ips = await resolveMultiDomainToIps(config.cfDomain, 1);
        if (ips.length > 0) ipv4Hints = ips.map(bytesToIp);
    } else {
        ipv4Hints = [DEFAULT_CF_IP];
    }
    
    if (!isDomainIpv4Only(domain)) {
        if (config.ip6) {
            ipv6Hints = parseIpList(config.ip6);
        } else if (config.cfDomain) {
            const ips = await resolveMultiDomainToIps(config.cfDomain, 28);
...
async function packMetaEchWithHints(domain, config) {
    ...
    if (config.metaIp4) {
        ipv4Hints = parseIpList(config.metaIp4);
    } else {
        ipv4Hints = [DEFAULT_META_IP];
    }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .result-box.error { color: var(--error); border-color: rgba(239, 68, 68, 0.3); }
        .result-box.success { border-color: rgba(16, 185, 129, 0.2); }
        ...
        .cf-mode { --mode-accent: var(--cf); --mode-glow: rgba(246, 130, 31, 0.15); }
        .meta-mode { --mode-accent: var(--meta); --mode-glow: rgba(6, 104, 225, 0.15); }
...
<body>
    ...
    <script>
        function onModeChange() {
            ...
            cfParams.classList.toggle('show', mode === 'cf');
            metaParams.classList.toggle('show', mode === 'meta');
        }
        ...
        function showResult(message, type) {
            ...
            if (!message) resultDiv.style.display = 'none';
        }
    </script>
</body>
</html>`;
