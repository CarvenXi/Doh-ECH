 ...
 * 最终版：区分 Meta 优选 IP 参数，静态 Meta 域名，上游竞速，测速排序
 */
...
const API_PATH = '/ech';        // 返回携带 ECH 的记录
const TEST_PATH = '/doh';       // 纯净无 ECH 转发
...
const DEFAULT_TWITTER_IP = "104.18.10.118";   // 单个默认 IP

const META_DOMAINS = [
    "facebook.com", "messenger.com", "instagram.com", "whatsapp.com", "fb.com", "meta.com"      // 可按需增减
];
const DEFAULT_META_IP = "157.240.1.35";       // 示例 Meta 边缘 IP
    ...
    try {
        ...
        if (isTwitter || isMeta) {
            ...
            if (qType === 65) {
                if (isTwitter) {
                    const echRdata = await fetchCleanEchRdata(config.echDomain, ctx);
                    if (echRdata) return dnsResponse(createMultiAnsResponse(id, qName, 65, [echRdata], 3600));
                    return dnsResponse(createMultiAnsResponse(id, qName, 65, [], 60));
                }
                if (isMeta) {
                    const echRdata = packHttpsParams(1, ".", [
            ...
            if (qType === 1) {
                ...
                if (isTwitter && config.ip4) {
                    ...
                    // 兼容旧行为：通过 cfDomain 解析 IP（仅 Twitter）
                    const cfIps = await fetchReplacementIps(config.cfDomain, 1, ctx);
                    if (cfIps) ipStrings = cfIps.map(ip => bytesToIp(ip));
                } else {
                    // 使用默认 IP
                ...
                if (ipStrings.length > 0) {
                    const sorted = await orderIpsByLatency(ipStrings, qName);
                    const finalBytes = sorted.map(ipToBytes);
        ...
        // A / AAAA 处理（CIDR 匹配的域名，分别使用对应 IP 参数）
        if (qType === 1 || qType === 28) {
            // Cloudflare 域名：ip4 / ip6 优先
            if (ownerData === 'CF') {
                if (qType === 1 && config.ip4) {
                    const ips = parseIpList(config.ip4).map(ipToBytes);
                    return dnsResponse(createMultiAnsResponse(id, qName, 1, ips, 300));
                }
                if (qType === 28 && config.ip6) {
                    const ips = parseIpList(config.ip6).map(ipv6ToBytes);
                    return dnsResponse(createMultiAnsResponse(id, qName, 28, ips, 300));
                }
                if (config.cfDomain) {
            ...
            // Meta 域名（CIDR 匹配）：metaIp4 / metaIp6 优先
            if (ownerData === 'META') {
                if (qType === 1 && config.metaIp4) {
                    const ips = parseIpList(config.metaIp4).map(ipToBytes);
                    return dnsResponse(createMultiAnsResponse(id, qName, 1, ips, 300));
                }
                if (qType === 28 && config.metaIp6) {
                    const ips = parseIpList(config.metaIp6).map(ipv6ToBytes);
                    return dnsResponse(createMultiAnsResponse(id, qName, 28, ips, 300));
                }
                // 若无自定义，使用探测到的 IP 或转发查询
                let rawIps = [];
...
// ============================================================================
// IP 延迟排序（并发测速，总等待 ≤ 1.2 秒）
// ============================================================================
async function orderIpsByLatency(ipList, hostname, maxWait = 1200) {
    if (ipList.length <= 1) return ipList;

    const start = Date.now();
    const promises = ipList.map(ip => {
        return (async () => {
            const ctrl = new AbortController();
            const timer = setTimeout(() => ctrl.abort(), maxWait);
            try {
                await fetch(`http://${ip}/`, {
                    method: 'HEAD',
                ...
                clearTimeout(timer);
                return { ip, latency: Number.MAX_SAFE_INTEGER };
            }
        })();
    });
    ...
    for (const res of results) {
        if (res.status === 'fulfilled') measured.push(res.value);
    }
    const successIps = new Set(measured.map(m => m.ip));
    for (const ip of ipList) {
        if (!successIps.has(ip)) remaining.push(ip);
    }

    measured.sort((a, b) => a.latency - b.latency);
    return [...measured.map(m => m.ip), ...remaining];
}
...
async function forwardQuery(body) {
    ...
    const pGoogle = fetch(UPSTREAM_DNS_GOOGLE, reqInit).then(res => {
        if (res.ok) return res;
        throw new Error(`Google DNS error: ${res.status}`);
    });
    const pAli = fetch(UPSTREAM_DNS_ALI, reqInit).then(res => {
        if (res.ok) return res;
        throw new Error(`Ali DNS error: ${res.status}`);
    });

    try {
        return await Promise.any([pGoogle, pAli]);
    } catch (e) {
        console.error('All upstreams failed, falling back to Google DNS');
...
function createMultiAnsResponse(id, qn, qt, rds, ttl = 3600) {
    ...
    const pointer = 0xC000 | 12; // 指向问题部分域名开始
    let totalLen = questionLen;
    for (const r of rds) {
        totalLen += 2 + 2 + 2 + 4 + 2 + r.length; // pointer(2)+type(2)+class(2)+ttl(4)+rdlen(2)+rdata
    }

    const buf = new Uint8Array(totalLen);
    const v = new DataView(buf.buffer);
    v.setUint16(0, id);
    v.setUint16(2, 0x8180); // QR=1, RD=1, RA=1
    v.setUint16(4, 1);       // QDCOUNT
    v.setUint16(6, rds.length); // ANCOUNT
    v.setUint16(8, 0);
    v.setUint16(10, 0);
    ...
    v.setUint16(offset, 1);  offset += 2; // QCLASS IN

    for (const r of rds) {
        v.setUint16(offset, pointer); offset += 2; // 域名指针
        v.setUint16(offset, qt); offset += 2;
        v.setUint16(offset, 1); offset += 2; // CLASS IN
