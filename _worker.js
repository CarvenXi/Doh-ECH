/**
 * DOH-ECH Pages（带 Cache API 缓存）
 * 路由：  /          -> 前端查询测试页面
 *        /api/query -> JSON API
 *        /ech       -> DoH 注入 ECH
 *        /doh       -> DoH 纯转发
 * 缓存策略：上游 DNS JSON 结果缓存到 Edge（A/AAAA 300s，HTTPS 1800s）
 */
...
// ==========仅支持IPv4的域名（不返回 AAAA 记录）==========
const IPV4_ONLY_DOMAINS = [
    "twitter.com", "x.com", "t.co", "twimg.com"
];

const DEFAULT_CF_IP = "104.18.10.118";   // Cloudflare 默认优选 IP4
const DEFAULT_CF_IP6 = "2606:4700::6812:a76"; //Cloudflare 默认优选 IP6
...
<body>
    <div class="container">
        <h1>🔒 DNS 记录查询</h1>
        <p class="subtitle">自动为 CF/META 查询记录注入ECH配置，可优选IP，cf支持多域名优选，支持Edge缓存加速</p>
        <div>
